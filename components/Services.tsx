...
export default function Services({ onView }: card) {
  ...
  "
>
  {sliceCategoriesData.map((category) => (
    <Motion
      key={category.id}
    ...
    >
      <div className="flex flex-col h-full cursor-pointer"  onClick={() => handleView(category.id!)}>
        ...
        <div className="p-4 sm:p-5 flex flex-col flex-1">
            ...
            {category.subServices.length > 3 && (
              ...
              >
                {expandedId === category.id
                  ...
                  : `+${category.subServices.length - 3} ${t(
                      "servicesSection.showMore",
                    )}`}
              </button>
            )}
          </div>
          ...
          <button
            ...
            className="mt-auto text-teal-600 text-xs sm:text-sm font-semibold inline-flex items-center justify-center gap-1 group-hover:gap-2 transition-all"
          >
